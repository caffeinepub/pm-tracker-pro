...
import { AlertCircle, Eye, EyeOff, Settings, UserPlus } from "lucide-react";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    getRawActor()
      .then((a) => a.getUserCount())
      .then((count: bigint) => {
        setIsFirstRun(Number(count) === 0);
      })
      .catch(() => {
        setIsFirstRun(false);
      });
  ...
  const handleSetup = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!setupUsername.trim() || !setupName.trim() || !setupPassword) {
    ...
    try {
      ...
      const ok = await a.setupInitialAdmin(
        setupUsername.trim(),
        btoa(setupPassword),
        setupName.trim(),
      );
      if (ok) {
        ...
        setError(
          "Setup failed — an admin account may already exist. Please refresh.",
        );
      }
    } catch {
      ...
      >
        ...
        {isFirstRun === null && (
          <div className="industrial-card p-8 text-center">
            ...
            <p style={{ color: "oklch(0.68 0.010 260)" }}>Connecting...</p>
          </div>
        )}
        ...
        {isFirstRun === true && (
          <div className="industrial-card p-8">
            ...
            <p
              ...
              style={{ color: "oklch(0.68 0.010 260)" }}
            >
              No admin account found. Create your administrator account to get
              started.
            </p>

            <form onSubmit={handleSetup} className="space-y-4">
              <div className="space-y-1.5">
                <Label style={{ color: "oklch(0.68 0.010 260)" }}>
                  Full Name
                </Label>
                <Input
                  type="text"
